...
class AverageRecall(Metric):
	...
	def __init__(self, iou_threshold_min: float = 0.5, iou_threshold_max: float = 1.0):
		...
		assert iou_threshold_max >= iou_threshold_min, "max value must be greater or equal than min value"
	...
	def compute(self):
		...
		average_recall = torch.trapz(recalls, self.thresholds) / (self.iou_threshold_max - self.iou_threshold_min)

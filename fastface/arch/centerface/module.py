...
class CenterFace(nn.Module):
    ...
    def __init__(self, config: Dict, **kwargs):
        ...
        self.register_buffer(
            "grids",
            utils.box.generate_grids(1500/self.output_stride, 1500/self.output_stride),
            persistent=False
        )

        # define losses
        self.cls_loss = BinaryFocalLoss(
            gamma=config["cls_loss"]["gamma"],
            alpha=config["cls_loss"]["alpha"]
        )
    ...
    def logits_to_preds(self,
        ...
        centers = (self.grids[:fh, :fw, :].repeat(batch_size, 1, 1, 1) + 0.5 + offset_regs) * self.output_stride
        ...
        return torch.cat(
            [x1y1, x2y2, heatmap.unsqueeze(3)], dim=3
    ...
    def compute_loss(self,
        ...
        target_cls, target_offset, target_wh = self.build_targets(
            max(fh, fw),
            raw_targets,
            dtype=offset_regs.dtype,
            device=offset_regs.device
        )

        mask = target_cls != 1
        num_positives = (~mask).sum()
        cls_loss = self.cls_loss(cls_logits, target_cls)
        cls_loss[num_positives:] = torch.pow(1 - target_cls[mask], self.beta_val) * cls_loss[num_positives:]
    ...
    def build_targets(
        ...
        target_cls = torch.zeros(*(batch_size, fmap_size, fmap_size), dtype=dtype, device=device)
        target_offset = torch.zeros(*(batch_size, fmap_size, fmap_size, 2), dtype=dtype, device=device)
        target_wh = torch.zeros(*(batch_size, fmap_size, fmap_size, 2), dtype=dtype, device=device)
        ...
        for i, targets in enumerate(raw_targets):
            ...
            for j, center in enumerate(fmap_centers):
                # center: x, y
                target_w = (targets["target_boxes"][j, 2] - targets["target_boxes"][j, 0]).cpu().item()
                target_h = (targets["target_boxes"][j, 3] - targets["target_boxes"][j, 1]).cpu().item()
                sigma = (
                    gaussian_radius((target_h, target_w), min_overlap = 0.7) * 2 + 1
                ) / 6
                heatmap = utils.kernel.get_gaussian_kernel(fmap_size,
                    sigma=sigma, center_point=center.cpu().numpy(), normalize=False)
            ...
            target_wh[i, true_indexes_y, true_indexes_x, 0] = torch.log(w / self.output_stride + 1e-8)
            target_wh[i, true_indexes_y, true_indexes_x, 1] = torch.log(h / self.output_stride + 1e-8)

            # set target offset
            target_offset[i, true_indexes_y, true_indexes_x, :] = (centers / self.output_stride) - fmap_centers
    ...
    def configure_optimizers(self, hparams: Dict = {}):
        optimizer = torch.optim.Adam(
            self.parameters(),
            lr=hparams.get("learning_rate", 5e-4)
        )

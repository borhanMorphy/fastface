...
def apply_conv2d(img: np.ndarray, kernel: np.ndarray) -> np.ndarray:
    assert len(kernel.shape) == 2, "kernel shape must be 2D but found {}".format(len(kernel.shape))
    ...
    for ch in range(img.shape[2]):
        subM = np.lib.stride_tricks.as_strided(img[..., ch], shape = s, strides = img[..., ch].strides * 2)

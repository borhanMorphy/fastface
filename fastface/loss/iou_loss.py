...
class DIoULoss(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        intersection = (
            # min_x2 - max_x1
            (torch.min(input[:, 2], target[:, 2]) - torch.max(input[:, 0], target[:, 0])).clamp(0) * \

            # min_y2 - max_y1
            (torch.min(input[:, 3], target[:, 3]) - torch.max(input[:, 1], target[:, 1])).clamp(0)
        )

        # union area
        input_wh = input[:, [2, 3]] - input[:, [0, 1]]
        target_wh = target[:, [2, 3]] - target[:, [0, 1]]
        union = (input_wh[:, 0] * input_wh[:, 1]) + (target_wh[:, 0] * target_wh[:, 1]) - intersection

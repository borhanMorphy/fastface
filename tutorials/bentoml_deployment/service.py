...
@artifacts([OnnxModelArtifact("model", backend="onnxruntime")])
class FaceDetectionService(BentoService):
    ...
    def detect(self, imgs: List[np.ndarray]):
        ...
        preds = []
        for img in imgs:
            results = self.artifacts.model.run(
                None, {input_name: self.prepare_input(img)}
            )[0]
            preds.append(self.to_json(results))
        return preds

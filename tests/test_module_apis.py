...
def test_module_deploy_to_torchscript(arch: str, config: str):
    ...
    assert isinstance(sc_module, torch.jit.ScriptModule), "build failed \
        for {} with config {}".format(arch, config)
...
def test_module_deploy_to_onnx(arch: str, config: str):
    ...
    assert len(ort_output.shape) == 2, "shape of the output must be length of 2 but found {}".format(len(ort_output.shape))
    assert ort_output.shape[1] == 6, "shape of output must be N,6 but found N,{}".format(ort_output.shape[1])
...
class WiderFaceAP(Metric):
	...
	def compute(self) -> float:
		...
		# [0] + propose + [0]
		propose = np.concatenate([[0.], propose, [0.]])
	...
	def evaluate_single_image(self, preds:np.ndarray, gts:np.ndarray,
			ignore:np.ndarray) -> Tuple[np.ndarray,np.ndarray]:
	...
	def calculate_image_pr(self, preds:np.ndarray, ignore_pred_mask:np.ndarray,
			match_counts:np.ndarray) -> np.ndarray:
